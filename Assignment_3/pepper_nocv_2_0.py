...
## 2.1 	Added LED commands for left and right leds
## 	Added tablet service proxy and setbackgroundcolor command
...
def BumperTouch():
    bumperA_touch = memoryProxy.getData("Device/SubDeviceList/Platform/FrontRight/Bumper/Sensor/Value", 0)
    bumperB_touch = memoryProxy.getData("Device/SubDeviceList/Platform/FrontLeft/Bumper/Sensor/Value", 0)
    bumperC_touch = memoryProxy.getData("Device/SubDeviceList/Platform/Back/Bumper/Sensor/Value", 0)
    bumper = bumperA_touch or bumperB_touch or bumperC_touch
    return bumper
...
def InitProxy(IP="marvin.local", proxy=[0], PORT = 9559):
    ...
    ALModuleList=["ALTextToSpeech","ALAudioDevice","ALMotion","ALMemory","ALFaceDetection","ALVideoDevice","ALLeds","ALFaceTracker","ALSpeechRecognition","ALAudioPlayer","ALVideoRecorder","ALSonar","ALRobotPosture","ALLandMarkDetection","ALTracker","ALSoundDetection","ALAudioSourceLocalization", "ALTabletService"]
    proxyDict={}
    ...
    tabletProxy=proxyDict["ALTabletService"]
...
## Changed by Tim last on 08-05-2018 ##
## Added eye and ear led left/right  ##
...
def EarLED(color=[0,0,0],interpol_time = 0, POST=True):
    sGroup = "EarLeds"
    try:
        if POST:
            ledProxy.post.fadeRGB(sGroup, 256*256*color[0] + 256*color[1] + color[2],interpol_time)
...
def EyeLEDLeft(color=[0,0,0],interpol_time = 0, POST=True):
    sGroup = "LeftFaceLeds"
    try:
        if POST:
            ledProxy.post.fadeRGB(sGroup, 256*256*color[0] + 256*color[1] + color[2],interpol_time)
...
def EarLEDLeft (color=[0,0,0],interpol_time = 0, POST=True):
    sGroup = "LeftEarLeds"
    try:
        if POST:
            ledProxy.post.fadeRGB(sGroup, 256*256*color[0] + 256*color[1] + color[2],interpol_time)
...
def EyeLEDRight (color=[0,0,0],interpol_time = 0, POST=True):
...
def EarLEDRight (color=[0,0,0],interpol_time = 0, POST=True):
    sGroup = "RightEarLeds"
    try:
        if POST:
            ledProxy.post.fadeRGB(sGroup, 256*256*color[0] + 256*color[1] + color[2],interpol_time)
...
## Added by Tim on 24-05-18: waituntilmoveisfinished ##
################################################################################
...
def ExtCollisionProtection(name="Move", enable=False):
...
## Added by Tim on 09-05-18 ##
## Tablet services ##

def TurnScreenOff (isOn=False):
	tabletProxy.turnScreenOn(False)
